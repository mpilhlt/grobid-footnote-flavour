...
In this flavour references are mixed with footnotes with three different styles: 
...
1. Mixed reference (single reference + commentary) -> one citation with the commentary in the `<note>`
2. Mixed reference (multiple references + multiple commentary) -> TBD

![complex-reference.png](complex-reference.png)

3. References to other references (`supra note`, `infra note`) -> added as `<note>`, not processed at the moment